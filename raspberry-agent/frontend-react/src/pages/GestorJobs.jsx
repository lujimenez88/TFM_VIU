...
import { getDronesDisponibles, getJobsDisponibles, crearJob, actualizarJob } from '../services/api';
...
const GestorJobs = () => {
  ...
  const finalizarJob = async (id) => {
    const confirm = window.confirm("¿Estás seguro de finalizar este job?");
    if (!confirm) return;
    const res = await actualizarJob(id, { estado: 'finalizado' });
    if (res?.id) cargarJobs();
    else alert("No se pudo finalizar el job");
  };
          ...
          {jobs.map((job, i) => (
            <tr key={job.id} style={{ backgroundColor: i % 2 === 0 ? '#fff' : '#f9f9f9' }}>
              ...
              <td style={{ padding: '8px', border: '1px solid #ddd' }}>
                {job.estado === 'activo' && (
                  <button onClick={() => finalizarJob(job.id)} style={{ backgroundColor: '#dc3545', color: 'white', border: 'none', padding: '6px 10px', borderRadius: '5px' }}>Finalizar</button>
                )}
              </td>
            </tr>
          ))}
